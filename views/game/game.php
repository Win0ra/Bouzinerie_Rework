...
$questionByQuizz = $questionRepository->findBy(['quiz' => $quiz]);

// Vérifie que les questions existent
if (empty($questionByQuiz)) {
    die("Pas de questions trouvées pour ce quiz");
}

// Valide le numéro de page
$totalQuestions = count($questionByQuiz);
...
$nextPageParam = $nextPage ? "?quiz_id={$quiz_id}&page={$nextPage}" : "#";

// // Optionnel : récupérer l'image du quiz
// $quizImage = method_exists($quiz, 'getImage') ? $quiz->getImage() : "./../assets/img/Teldrassil.jpg";
// ?>
    ...
    <?php } ?>
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                ...
                let correctAnswers = parseInt(localStorage.getItem('correctAnswers_<?php echo $quiz_id; ?>')) || 0;
        ...
        // Réinitialiser le compteur si le quiz est relancé
                if (reset === 'true') {
                    localStorage.setItem('correctAnswers_<?php echo $quiz_id; ?>', 0);
                    correctAnswers = 0;
                }
                ...
                document.querySelectorAll('.answer').forEach(answer => {
                    answer.addEventListener('click', function(e) {
                        ...
                        if (this.getAttribute('data-correct') === 'true') {
                            correctAnswers++;
                            localStorage.setItem('correctAnswers_<?php echo $quiz_id; ?>', correctAnswers);
                            scoreElement.textContent = `Answers: ${correctAnswers}/<?php echo $totalQuestions; ?>`;
                        }
